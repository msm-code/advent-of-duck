...
def nextsteps(state):
    pos, dir, speed = state
    opts = [(follow(d, pos), d, 1) for d in turns[dir]]
    if speed < 3: opts.append((follow(dir, pos), dir, speed + 1))
...
def bestxy(x, y): return min(heat for move, heat in bestheat.items() if move[0] == (x, y))
