...
effect = 0
for b in range(len(laybricks)):
    removed = set([b])
    while True:
        for bi, nb in lays_on.items():
            if bi in removed: continue
            if nb and all(n in removed for n in nb):
                removed.add(bi)
                break
        else: break
    if len(removed) > 1: effect += len(removed) - 1
print(effect)
