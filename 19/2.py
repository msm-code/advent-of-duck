...
def rotations(vec):
    unique = set(next(iter(vec)))  # next(iter(set)) == first element
        ...
        for pa in a:
            for pb in b:
                ...
                if sum(vadd(p, shift) in a for p in b) >= 12:
                    return a | set(vadd(p, shift) for p in b if vadd(p, shift) not in a), shift
    return None
...
for px in shifts:
    for py in shifts:
        maxdist = max(maxdist, sum(abs(c) for c in vsub(px, py)))
print(maxdist)